angular.module('kityminderEditor').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('ui/directive/noteEditor/noteEditor.html',
    "<div class=\"panel panel-default\" ng-init=\"noteEditorOpen = false\" ng-show=\"noteEditorOpen\"><div class=\"panel-heading\"><i class=\"close-note-editor iconfont icon-guanbi\" ng-click=\"closeNoteEditor()\"></i><h3 class=\"panel-title\">备注</h3></div><div class=\"panel-body\"><div ng-show=\"noteEnabled\" ui-codemirror=\"{ onLoad: codemirrorLoaded }\" ng-model=\"noteContent\" ui-codemirror-opts=\"{\r" +
    "\n" +
    "                gfm: true,\r" +
    "\n" +
    "                breaks: true,\r" +
    ...
    "                lineNumbers:true\r" +
    "\n" +
    "             }\"></div><p ng-show=\"!noteEnabled\" class=\"km-note-tips\">请选择节点编辑备注</p><div class=\"km-note-remove\" ng-show=\"noteEnabled && visibleRemoveBtn()\"><div class=\"km-note-remove-btn\" ng-click=\"minder.execCommand('note', null)\">移除当前备注</div></div></div></div>"
  );
  ...
  $templateCache.put('ui/dialog/imExportNode/imExportNode.tpl.html',
    ...
    "\n" +
    "    </textarea></div><div class=\"modal-footer\"><button class=\"btn btn-primary\" ng-click=\"ok()\" ng-disabled=\"type === 'import' && value == ''\">OK</button> <button class=\"btn btn-default\" ng-click=\"cancel()\">Cancel</button></div>"
  );


  $templateCache.put('ui/dialog/image/image.tpl.html',
    "<div class=\"modal-body\"><div class=\"dialog-header\"><button type=\"button\" class=\"close\" ng-click=\"cancel()\"><i class=\"iconfont icon-guanbi\"></i></button><div class=\"dialog-title\">插入图片</div></div><div class=\"km-tabs\"><div class=\"km-tab\" ng-click=\"toggleTab(tabs.upload)\" ng-class=\"{active: activeTab === tabs.upload}\">上传图片</div><div class=\"km-tab\" ng-click=\"toggleTab(tabs.link)\" ng-class=\"{active: activeTab === tabs.link}\">在线图片</div></div><form ng-show=\"activeTab === tabs.upload\"><div class=\"form-group\"><input type=\"file\" name=\"upload-image\" id=\"upload-image\" class=\"upload-image\" accept=\".jpg,.JPG,jpeg,JPEG,.png,.PNG,.gif,.GIF\" onchange=\"angular.element(this).scope().uploadImage()\"><label for=\"upload-image\" class=\"select-file-btn km-btn km-btn-primary\"><span>选择文件</span></label></div><div class=\"form-group image-preview-box\"><span ng-show=\"!data.url\" class=\"image-preview-label\">图片预览区域</span> <img class=\"image-preview\" id=\"image-preview\" ng-src=\"{{ data.url }}\" alt=\"{{ data.title }}\"></div></form><form ng-show=\"activeTab === tabs.link\"><div class=\"form-group\" ng-class=\"{true: 'has-success', false: 'has-error'}[urlPassed]\"><input type=\"text\" class=\"form-control\" ng-model=\"data.url\" ng-blur=\"urlPassed = data.R_URL.test(data.url)\" ng-focus=\"this.value = data.url\" ng-keydown=\"shortCut($event)\" id=\"image-url\" placeholder=\"必填：请复制图片地址（http(s)://开头）到这里\"></div><div class=\"form-group image-preview-box\"><span ng-show=\"!data.url\" class=\"image-preview-label\">图片预览区域</span> <img class=\"image-preview\" id=\"image-preview\" ng-src=\"{{ data.url }}\" alt=\"{{ data.title }}\"></div></form><div class=\"km-btns\"><button class=\"km-btn km-btn-default\" ng-click=\"cancel()\">取消</button> <button class=\"km-btn km-btn-primary\" ng-click=\"ok()\">确定</button></div></div>"
  );
