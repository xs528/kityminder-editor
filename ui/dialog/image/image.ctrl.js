angular.module('kityminderEditor').controller('image.ctrl', [
  ...
  'server',
  'editbarService',
  function($http, $scope, $modalInstance, image, server, editbarService) {
    $scope.tabs = {
      link: 'link',
      upload: 'upload',
    }
    $scope.activeTab = $scope.tabs.upload
    ...
    $scope.uploadImage = function() {
      ...
      if (/^.*\.(jpg|JPG|jpeg|JPEG|gif|GIF|png|PNG)$/.test(fileInput.val())) {
        ...
        return server.uploadFile(file).then(function(url) {
          if (url) {
            $scope.data.url = url
            $scope.$evalAsync()
          }
